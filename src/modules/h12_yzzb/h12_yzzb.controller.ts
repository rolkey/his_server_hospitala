...
export class h12_yzzbController {
  ...
  async sign(
    ...
    const { zyid, yzxh, yzlx, yzzh, userId, zcid, jsys, jsysZcid, fgqm, kssqm } = data;
    return this.h12_yzxbService.sign(
      zyid,
      yzxh,
      yzlx,
      yzzh,
  ...
  //   @UseGuards(JwtGuard)
  async reviewBack(
    @Body() dto: { zyid: string; yzlx: number; yzzh: number[]; info: string; userId: string },
  ) {
    return await this.h12_yzxbServiceNew.reviewBack(dto);
  }
