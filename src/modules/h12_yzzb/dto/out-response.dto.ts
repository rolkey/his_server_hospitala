...
export interface OutResponse {
  ...
  errorDetails?: Array<{   // 错误明细列表（当isDetails为true时包含）
    yzlx?: string;         // 医嘱类型
    mxxh?: string;         // 明细号
    yzrq?: string;         // 医嘱日期
    xmid?: string;         // 项目ID
...
// 创建错误响应
export function createErrorResponse(message: string, errorCode?: number, isDetails: boolean = false, errorDetails?: Array<any>): OutResponse {
  return {
    // 操作是否成功
    success: false,
    // 错误信息或成功提示
