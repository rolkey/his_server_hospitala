...
export class AuthController {
  ...
  // @UseGuards(LocalGuard)
  // @Post('loginMobile')
  // async loginMobile(@Req() req: any, @Body() body: any) {
  //   return this.authService.login(req.user, req.session?.code);
  // }
  @Post('register')
  ...
  async changePassword(@Req() req: any, @Body() body: ChangePasswordDto) {
    const ret = await this.authService.validateUser(req.user.usid, body.oldPassword);
    if (!ret) {
      throw new CustomException(ErrorCode.ERR_10004);
    }
    // 修改密码
    await this.userService.resetPassword(req.user.usid, body.newPassword);
    // 修改密码后退出登录
